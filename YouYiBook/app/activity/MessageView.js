...
const BASEURL = 'http://121.42.238.246:8080/unitrip_bookstore/bookstore/queryMessage'
const DELETEBASEURL = 'http://121.42.238.246:8080/unitrip_bookstore/bookstore/DelMessage'
var pageNum = 0;
...
const FITERIMG = require('../img/btn_title_clean.png');
export default class MessageActivity extends Component {
    ...
    fetchData(params) {
        ...
        NetUitl.post(BASEURL, params, '', function (responseData) {
            ...
            if (null != responseData && responseData.return_code == '0') {
                ...
                that.addItemKey(responseData.messages);
                pageNum++;
            } else {
                that.setState({
                    show: false
    ...
    deleteData() {
        ...
        let params = StringBufferUtils.toString();
        this.DeletefetchData(params);

    }

    // 数据请求
    DeletefetchData(params) {
        var that = this;
        console.log(DELETEBASEURL + params);
        NetUitl.post(DELETEBASEURL, params, '', function (responseData) {
            //下面的就是请求来的数据
            if (null != responseData && responseData.return_code == '0') {
                console.log(responseData);
                toastShort('删除成功！');
                ...
                <TouchableOpacity onPress={() => this.clickItem(itemData, index)} activeOpacity={0.8}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <View style={{ height: 100, flexDirection: 'column', justifyContent: 'center' }}>
                            <Text style={styles.news_item_title} numberOfLines={1}>{itemData.title}</Text>
                            <Text style={styles.news_item_title} numberOfLines={2}>{itemData.content}</Text>
                            <View style={{ width: width, flexDirection: 'row', justifyContent: 'flex-end', marginRight: 10 }}>
                                <Text style={styles.rule_item_time} numberOfLines={1}>{itemData.message_time}</Text>
                            </View>
                        </View>
    ...
    render() {
        return (
            ...
            <View style={styles.main_view}>
                ...
                />
                <PublicTitle _backOnclick={() => this.backOnclik()} _finishOnlcik={() => this.finishOnlcik()} title='系统消息' finishIcon={FITERIMG} leftIcon={BACKICON} imgWidth={39} imgHeight={30} />
